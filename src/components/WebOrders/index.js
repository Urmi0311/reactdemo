...
export const WebOrders = () => {
  ...
  useEffect(() => {
    const db = firebase.database().ref(`users/${sessionStorage.getItem('uid')}`)
    db.once('value')
      .then(async (doc) => {
        const data = doc.val()
        if (
          ...
          alert('Error: This account does not have a store associated with it.')
          setWaiting(false)
        } else if (data === null) {
  ...
  const handleSelect = (e) => {
    ...
    setShowPrescription(country === 'AU' ? true : false)
  ...
  const handleLoad = async () => {
    ...
    if (orderObjByOrderNumber[orderNumber] !== undefined) {
      ...
      const _orderObj = {}
      let data = obj.products.map((item) => {
        const obj = {
          name: item.name,
          sku: item.sku,
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      alert(`Successfully uploaded image for order number ${uploadOrderNumber}`)
      setRequirePicture((old) => {
        return old.filter((c) => c.orderNumber !== uploadOrderNumber)
      })
    } catch (err) {
  ...
  const handleFinish = async () => {
    ...
    let cleanedTrackingNumber = undefined
    ...
    <>
                      ...
                      return (
                        <li key={index}>
                          ...
                          <ul>
                            ...
                            <li>{`Requested Pickup Time: ${moment(
                              value.pickupDate
                            ).format('DD MMM YYYY HH:mm')}`}</li>
                            <li>{`Job Number: ${
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                  ...
                                  if (check === true) {
                                    ...
                                    alert("The order already assigned to other store. Please contact Online Team.")
                                  }
                                  else{
      ...
      >
        ...
        <DialogContent>
          ...
          <h3 style={{color:'red'}}>Generating ticket is not working now. Please ask online team ("Web order" Group on Team) to get shipping label</h3>
          {currentOrderObj !== null &&
          currentOrderObj?.websiteType === 'ShoshaNZ' &&
          automaticTicket ? (
            <>
